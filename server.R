...
library(pROC)
library(googleVis)
library(plotly) #run this command alone if error on startup and try again
suppressPackageStartupMessages(library(googleVis))

fdata1 <- read.csv("Text Analysis Results.csv", sep = ",")
df <- read.csv("data_potential_predictors_train_SMOTE.csv", stringsAsFactors = FALSE)
df_test <- read.csv("data_potential_predictors_test.csv", stringsAsFactors = FALSE)
...
shinyServer(function(input, output) {
  ...
  output$textanalysis <- renderText({ 
    paste("Text Analysis of Loan Description Result")
  })
